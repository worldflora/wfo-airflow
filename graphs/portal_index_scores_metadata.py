...
@dag(
    ...
    params={"index_all":
        Param( 
            type="boolean",
            title="Reindex all the scores BEWARE!", 
            default=False,
            description="By default this will only index the scores metadata that has changed since the last run. This will delete scores in index and rebuild them one thousand at a time, which might take a while." )},
)
def portal_index_scores_metadata():
    """
    ### Update the facets metadata in the portal. Only does modified unless prompted otherwise.
    """

    @task()
    def delete_scores_metadata(**context):
        # if flagged to index all we simple index from the start of the epoc
        if context["params"]["index_all"]:
            print("Deleting all scores from index")
            portal = PortalApi(Variable.get("portal-api-url"), Variable.get("portal-api-token"))
            response = portal.deleteScoresMetadata()
